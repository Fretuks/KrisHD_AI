import {get, post} from "/app/api.js";
...
if (form) {
    ...
    if (session.error) status.textContent = session.error;
    ...
    document.querySelector("#showPassword").addEventListener("click", event => {
        ...
        event.currentTarget.textContent = show ? "Hide password" : "Show password";
    ...
    form.addEventListener("submit", async event => {
        ...
        if (result.error) { status.textContent = result.error; submit.disabled = false; return; }
